...
//将军令序列号检查
function checkSerial_new(formName, elementName, errTipName){
    var hasErr = false;
    var form = document.getElementById(formName);
	var elem = document.getElementById(elementName);
	var tip = document.getElementById(errTipName);
	...
	if(form && elem && tip){
        if(elem.value.length < 1){
		    hasErr = true;
		    tip.innerHTML = "请输入将军令序列号";
		    tip.style.display = "inline-block";
	    } else if(!patrn.test(value)){
	    	hasErr = true;
		    tip.innerHTML = "请输入正确格式的将军令序列号";
		    tip.style.display = "inline-block";
	   	}
        else{
		    tip.style.display = "none";
...
function checkPassword(formName, elementName1, elementName2, errTipName){
	...
	var tip = document.getElementById(errTipName);
    if(form && elem && tip){
      if(!(elem.value.length >= 6 && elem.value.length <= 16 && /^[\x00-\xff]+$/.test(elem.value))){
		hasErr = true;
		tip.innerHTML = "请输入正确的新密码";
		tip.style.display = "inline-block";
	  }else{
		if(isSimplePwd(elem.value)){
			hasErr = true;
			tip.innerHTML = "密码过于简单";
			tip.style.display = "inline-block";
		}
	    else if(elemName && elemName.value.indexOf(elem.value) != -1){
		    hasErr = true;
...
function checkMobile(formName, elementName, errTipName){
	...
	var tip = document.getElementById(errTipName);
	if(form && elem && tip){
      if(!/^(((13|15|18|17)\d{9})|(14\d{9}))$/.test(elem.value)){
		hasErr = true;
...
function checkFirstname2(formName, elementName, errTipName){
	...
	if(form && elem && tip){
      ...
      if(!/^[\u4e00-\u9fa5]{2,10}$/.test(elemValue)){
		hasErr = true;
		tip.innerHTML = "请输入2~10位汉字";
		tip.style.display = "inline-block";
	  }else{
		tip.style.display = "none";
	  }
...
//加入重复信息录入确认功能 20130617 by 郑弋天
function checkReInfo(p1, p2, tipId, type){
  var t1 = document.getElementById(p1).value,
      t2 = document.getElementById(p2).value,
  ...
  ;
  if(t1 != t2){
  	type && (tip.innerHTML = tipMsg[type]);
  	tip.style.display = "inline-block";
  	return false;
  }else{
  	tip.style.display = "none";
